...
pending2022 = list(filter(lambda records_data: records_data["RMA"].startswith('FS'),records_data))
pending2022
for d in pending2022:
    d['Receipt'] = d.pop('收货日期\nReceipt Date')
    d['PN'] = d.pop('产品型号\nP/N')
    d['Num'] = d.pop('数量\nNumber')
    d['Receiver'] = d.pop('签收人\nReceiver')
pending2022
def pending2022(request):
    return render(request,'pending2022.html',{'pending2022':pending2022})
